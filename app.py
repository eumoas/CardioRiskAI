...
=============================================================================
  ...
  Com persistência PostgreSQL e cadastro de pacientes
=============================================================================
"""
...
def get_db_connection():
    ...
    Prioridade: st.secrets > variáveis de ambiente > None (sem DB).
    """
    if psycopg2 is None:
        return None
    ...
    try:
        # Tentar variáveis de ambiente (Docker)
        db_host = os.environ.get('DB_HOST')
        if db_host:
            conn = psycopg2.connect(
                host=db_host,
                port=os.environ.get('DB_PORT', '5432'),
                dbname=os.environ.get('DB_NAME', 'cardiorisk_db'),
                user=os.environ.get('DB_USER', 'cardiorisk'),
                password=os.environ.get('DB_PASSWORD', 'cardiorisk123'),
                connect_timeout=5
            )
            return conn
    except Exception:
...
def save_patient(data: dict) -> bool:
    """Salva os dados do paciente no PostgreSQL."""
    conn = get_db_connection()
    if conn is None:
        return False
    try:
        ...
        cur.execute("""
            INSERT INTO pacientes (
                ...
                passos_diarios, horas_sono, agua_litros, calorias, horas_trabalho,
                freq_cardiaca, pressao_sistolica, pressao_diastolica, colesterol,
                fumante, alcool, historico_familiar,
                pratica_atividade, atividade_vezes_semana,
                risco_classificado, probabilidades
            ) VALUES (
                %(nome)s, %(idade)s, %(sexo)s, %(peso)s, %(altura)s, %(imc)s,
                %(passos_diarios)s, %(horas_sono)s, %(agua_litros)s, %(calorias)s, %(horas_trabalho)s,
                %(freq_cardiaca)s, %(pressao_sistolica)s, %(pressao_diastolica)s, %(colesterol)s,
                %(fumante)s, %(alcool)s, %(historico_familiar)s,
                %(pratica_atividade)s, %(atividade_vezes_semana)s,
                %(risco_classificado)s, %(probabilidades)s
            )
        """, data)
        conn.commit()
        cur.close()
...
def get_patients(limit: int = 50) -> list:
    """Recupera pacientes registrados."""
    conn = get_db_connection()
    if conn is None:
        return []
    try:
...
def get_patient_count() -> int:
    """Retorna o total de pacientes registrados."""
    conn = get_db_connection()
    if conn is None:
        return 0
    try:
...
def classificar_imc(imc: float) -> tuple:
    """Retorna a classificação e cor do IMC."""
    if imc < 18.5:
        return "Abaixo do peso", "#3b82f6"
    elif imc < 25.0:
        return "Normal", "#10b981"
    elif imc < 30.0:
        return "Sobrepeso", "#f59e0b"
    elif imc < 35.0:
        return "Obesidade I", "#ef4444"
    elif imc < 40.0:
        return "Obesidade II", "#dc2626"
    else:
        return "Obesidade III", "#a855f7"
...
if db_available:
    st.markdown(
        ...
        '<span class="db-badge db-connected">🟢 Banco de Dados Conectado</span>'
        '</div>',
        unsafe_allow_html=True
    )
else:
    st.markdown(
        '<div style="text-align:center;margin-bottom:1rem;">'
        '<span class="db-badge db-disconnected">🔴 Banco de Dados Indisponível — classificação funciona, mas dados não serão salvos</span>'
        '</div>',
        unsafe_allow_html=True
    )
...
with st.sidebar:
    ...
    with col_alt:
        altura = st.number_input("Altura (cm)", min_value=100.0, max_value=230.0,
                                 value=170.0, step=1.0, format="%.0f")

    # Calcular e exibir IMC
    imc = calcular_imc(peso, altura)
    imc_class, imc_color = classificar_imc(imc)
    st.markdown(f"""
    <div class="imc-card">
        <div class="imc-label">IMC Calculado</div>
        <div class="imc-value">{imc}</div>
        <div class="imc-class" style="color: {imc_color};">{imc_class}</div>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("### 🏃 Estilo de Vida")
    pratica_atividade = st.selectbox("Pratica atividade física?", ["Não", "Sim"])
    atividade_vezes_semana = 0
    if pratica_atividade == "Sim":
        atividade_vezes_semana = st.slider("Quantas vezes por semana?", 1, 7, 3)
...
if predict_button:
    if not nome_paciente.strip():
        st.warning("⚠️ Por favor, informe o **nome do paciente** na barra lateral.")
    else:
        input_data = {
            'Idade': idade,
            'IMC': imc,
        ...
        <div class="risk-card {risk_css}">
            ...
            <div style="margin-top: 1rem; font-size: 0.95rem; color: #94a3b8;">
                Paciente: <strong style="color: #f0f4f8;">{nome_paciente}</strong>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if db_available:
            ...
            patient_data = {
                ...
                'fumante': fumante == 'Sim',
                'alcool': alcool,
                'historico_familiar': historico_familiar == 'Sim',
                'pratica_atividade': pratica_atividade == 'Sim',
                'atividade_vezes_semana': atividade_vezes_semana,
                'risco_classificado': risk_label,
                'probabilidades': json.dumps(prob_mapped_save)
            }

            if save_patient(patient_data):
                st.success("✅ Dados do paciente salvos com sucesso!")
            else:
                st.warning("⚠️ Não foi possível salvar os dados no banco.")
...
# =============================================================================
#  TABS — INFORMAÇÕES DO MODELO + HISTÓRICO
# =============================================================================
st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
...
# ---- Tab 3: Histórico de Pacientes ----
with tab3:
    st.markdown("### 📋 Histórico de Pacientes Registrados")

    if not db_available:
        st.info("🔌 Banco de dados não conectado. O histórico de pacientes estará disponível quando o PostgreSQL estiver configurado.")
    else:
        # Botão de atualizar
        if st.button("🔄 Atualizar Histórico", key="refresh_history"):
            st.rerun()

        total_patients = get_patient_count()
        st.markdown(f"**Total de pacientes registrados:** {total_patients}")

        patients = get_patients(limit=50)

        if not patients:
            st.info("Nenhum paciente registrado ainda. Use o formulário na barra lateral para classificar um paciente.")
        else:
            # Tabela resumida
            ...
            display_cols = {
                ...
                'pratica_atividade': 'Atividade Física',
                'atividade_vezes_semana': 'Freq. Semanal',
                'data_registro': 'Data'
            }

            df_display = df_patients[[c for c in display_cols.keys() if c in df_patients.columns]].copy()
            ...
            # Formatar atividade física
            if 'Atividade Física' in df_display.columns:
                df_display['Atividade Física'] = df_display['Atividade Física'].map(
                    {True: '✅ Sim', False: '❌ Não'}
                )
            ...
            st.dataframe(
                ...
                column_config={
                    'Risco': st.column_config.TextColumn(
                        'Risco',
                        help='Classificação de risco cardiovascular'
                    ),
            ...
            risk_counts = df_patients['risco_classificado'].value_counts()
            for i, (risk_name, col) in enumerate(zip(
                ['Baixo', 'Moderado', 'Alto', 'Muito Alto'],
                [col_s1, col_s2, col_s3, col_s4]
            )):
                count = risk_counts.get(risk_name, 0)
                color = RISK_COLORS.get(risk_name, '#94a3b8')
                with col:
                    st.markdown(f"""
                    <div class="metric-card">
                        <div class="metric-mini">
                            <div class="value" style="color: {color};">{count}</div>
                            <div class="label">{risk_name}</div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            if len(risk_counts) > 0:
                fig_hist = go.Figure(data=[go.Pie(
                    ...
                    hole=0.5,
                    marker=dict(colors=[RISK_COLORS.get(r, '#94a3b8') for r in risk_counts.index]),
                    textinfo='label+value',
                    textfont=dict(size=12, color='white'),
                )])
                fig_hist.update_layout(
                    title=dict(text='Distribuição de Risco dos Pacientes', font=dict(color='#94a3b8', size=14)),
                    showlegend=False,
                    paper_bgcolor='rgba(0,0,0,0)',
                    plot_bgcolor='rgba(0,0,0,0)',
                    height=300,
...
with tab4:
    ...
    - **Antropométricos:** Peso, Altura, IMC (calculado automaticamente)
    - **Clínicos:** PA Sistólica/Diastólica, Colesterol, Freq. Cardíaca
    - **Estilo de vida:** Atividade física, Passos diários, Sono, Hidratação, Calorias, Horas de trabalho
...
<div style="text-align: center; padding: 1rem; color: #475569; font-size: 0.8rem;">
    ...
    Desenvolvido com Streamlit • PostgreSQL • MLflow • Docker • scikit-learn
</div>
""", unsafe_allow_html=True)
